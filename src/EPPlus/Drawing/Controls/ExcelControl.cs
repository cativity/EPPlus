...
public abstract class ExcelControl : ExcelDrawing
{
    ...
    internal ExcelControl(ExcelDrawings drawings,
                          XmlNode drawingNode,
                          ControlInternal control,
                          ZipPackagePart ctrlPropPart,
        ...
        : base(drawings, drawingNode, "xdr:sp", "xdr:nvSpPr/xdr:cNvPr", parent)
    {
        ...
        ZipPackageRelationship? rel =
            ws.Part.CreateRelationship(this.ControlPropertiesUri, TargetMode.Internal, ExcelPackage.schemaRelationships + "/ctrlProp");
    ...
    {
        string? xml =
            "<?xml version=\"1.0\" encoding=\"UTF-8\" standalone=\"yes\"?><formControlPr xmlns=\"http://schemas.microsoft.com/office/spreadsheetml/2009/9/main\" {0} />";
    ...
    internal string GetVmlAnchorValue()
    {
        ...
        return string.Format("{0}, {1}, {2}, {3}, {4}, {5}, {6}, {7}",
                             from.Column,
                             Math.Round(from.ColumnOff / EMU_PER_PIXEL * 1.5),
                             from.Row,
                             Math.Round(from.RowOff / EMU_PER_PIXEL * 1.5),
                             to.Column,
                             Math.Round(to.ColumnOff / EMU_PER_PIXEL * 1.5),
                             to.Row,
                             Math.Round(to.RowOff / EMU_PER_PIXEL * 1.5));
    }
        ...
        if (this.ControlType == eControlType.CheckBox || this.ControlType == eControlType.RadioButton)
        {
            ...
            if (c.Fill.Style != eVmlFillType.NoFill)
            {
                ExcelDrawingFill? fill =
                    new ExcelDrawingFill(this._drawings, this.NameSpaceManager, this.TopNode, this._topPath + "/xdr:spPr", this.SchemaNodeOrder);
