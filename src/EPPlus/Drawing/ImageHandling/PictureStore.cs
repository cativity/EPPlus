...
internal class PictureStore : IDisposable
{
        ...
        lock (this._images)
        {
            ...
            else
            {
                ...
                this._images.Add(hash,
                                 new ImageInfo()
                                 {
                                     Uri = uri, RefCount = 1, Hash = hash, Part = imagePart, Bounds = GetImageBounds(image, pictureType.Value, this._pck)
                                 });
            }
        }
    ...
    internal static string SavePicture(byte[] image, IPictureContainer container, ePictureType type)
    {
        ...
        container.RelPic =
            container.RelationDocument.RelatedPart.CreateRelationship(UriHelper.GetRelativeUri(container.RelationDocument.RelatedUri, container.UriPic),
                                                                      TargetMode.Internal,
