...
public abstract class ExcelSlicerCache : XmlHelper
{
    ...
    internal void CreatePart(ExcelWorkbook wb)
    {
        ...
        this.CacheRel = wb.Part.CreateRelationship(UriHelper.GetRelativeUri(wb.WorkbookUri, this.Uri),
                                                   TargetMode.Internal,
                                                   ExcelPackage.schemaRelationshipsSlicerCache);
    ...
    internal void CreateWorkbookReference(ExcelWorkbook wb, string uriGuid)
    {
        ...
        XmlNode? extNode =
            wb.GetOrCreateExtLstSubNode(uriGuid, prefix, new string[] { ExtLstUris.WorkbookSlicerPivotTableUri, ExtLstUris.WorkbookSlicerTableUri });
