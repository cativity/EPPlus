...
internal class EncryptedPackageHandler
{
    ...
    private MemoryStream EncryptPackageAgile(byte[] package, ExcelEncryption encryption)
    {
        ...
        xml +=
            "<p:encryptedKey spinCount=\"100000\" saltSize=\"16\" blockSize=\"16\" keyBits=\"256\" hashSize=\"64\" cipherAlgorithm=\"AES\" cipherChaining=\"ChainingModeCBC\" hashAlgorithm=\"SHA512\" saltValue=\"\" encryptedVerifierHashInput=\"\" encryptedVerifierHashValue=\"\" encryptedKeyValue=\"\" />";
    ...
    private static EncryptionInfoBinary CreateEncryptionInfo(string password, AlgorithmID algID, out byte[] key)
    {
        ...
        encryptionInfo.Header.KeySize = algID == AlgorithmID.AES128
                                            ? 0x80
                                            : algID == AlgorithmID.AES192
                                                ? 0xC0
                                                : 0x100;
    ...
    private MemoryStream DecryptAgile(EncryptionInfoAgile encryptionInfo, string password, long size, byte[] encryptedData, byte[] data)
    {
        if (encryptionInfo.KeyData.CipherAlgorithm == eCipherAlgorithm.AES)
        {
            ...
            if (IsPasswordValid(hashProvider, encr))
            {
                ...
                byte[]? key = DecryptAgileFromKey(encryptionInfo.KeyData,
                                                  encr.KeyValue,
                                                  encryptionInfo.DataIntegrity.EncryptedHmacKey,
                                                  encryptionInfo.KeyData.HashSize,
                                                  ivhmac);

                ivhmac = GetFinalHash(hashProviderDataKey, this.BlockKey_HmacValue, encryptionInfo.KeyData.SaltValue);

                byte[]? value = DecryptAgileFromKey(encryptionInfo.KeyData,
                                                    encr.KeyValue,
    ...
    private static MemoryStream DecryptBinary(EncryptionInfoBinary encryptionInfo, string password, long size, byte[] encryptedData)
    {
        ...
        if (encryptionInfo.Header.AlgID == AlgorithmID.AES128
            || (encryptionInfo.Header.AlgID == AlgorithmID.Flags
                && (encryptionInfo.Flags & (Flags.fAES | Flags.fExternal | Flags.fCryptoAPI)) == (Flags.fAES | Flags.fCryptoAPI))
    ...
    private static byte[] DecryptAgileFromKey(EncryptionInfoAgile.EncryptionKeyData encr, byte[] key, byte[] encryptedData, long size, byte[] iv)
    {
        ...
        ICryptoTransform decryptor = decryptKey.CreateDecryptor(FixHashSize(key, encr.KeyBits / 8), FixHashSize(iv, encr.BlockSize, 0x36));
...
#endif
    private static void EncryptAgileFromKey(EncryptionInfoAgile.EncryptionKeyEncryptor encr,
                                            byte[] key,
                                            byte[] data,
                                            long pos,
                                            long size,
                                            ...
                                            MemoryStream ms)
    {
        ...
        ICryptoTransform encryptor = encryptKey.CreateEncryptor(FixHashSize(key, encr.KeyBits / 8), FixHashSize(iv, 16, 0x36));
    ...
    private static byte[] GetPasswordHashBinary(string password, EncryptionInfoBinary encryptionInfo)
    {
        ...
        try
        {
            ...
            if (encryptionInfo.Header.AlgIDHash == AlgorithmHashID.SHA1
                || (encryptionInfo.Header.AlgIDHash == AlgorithmHashID.App && (encryptionInfo.Flags & Flags.fExternal) == 0))
            {
#if (Core)
    ...
    internal static ushort CalculatePasswordHash(string Password)
    {
        ...
        {
            hash ^= Password[i];
            hash = (ushort)((ushort)((hash >> 14) & 0x01) | (ushort)((hash << 1) & 0x7FFF)); //Shift 1 to the left. Overflowing bit 15 goes into bit 0
        }
