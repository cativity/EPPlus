...
internal partial class EpplusCssWriter : HtmlWriterBase
{
    ...
    internal void RenderAdditionalAndFontCss(string tableClass)
    {
        ...
        foreach (ExcelWorksheet? ws in worksheets)
        {
            ...
            this.WriteCssItem($"width:{ExcelColumn.ColumnWidthToPixels(Convert.ToDecimal(ws.DefaultColWidth), ws.Workbook.MaxFontWidth)}px;", this._settings.Minify);
            this.WriteClassEnd(this._settings.Minify);
    ...
    private void WriteFillStyles(ExcelFillXml f)
    {
        ...
        else
        {
            ...
            {
                this.WriteCssItem($"{PatternFills.GetPatternSvg(f.PatternType, this.GetColor(f.BackgroundColor), this.GetColor(f.PatternColor))}", this._settings.Minify);
            }
        }
    }
