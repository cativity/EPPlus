...
namespace OfficeOpenXml.Export.HtmlExport.Exporters
{
    internal class HtmlTableExporterAsync : HtmlRangeExporterAsyncBase
    {
            ...
            while (row <= endRow)
            {
                ...
                foreach (int col in _columns)
                {
                    ...
                    if (cell.Hyperlink == null)
                    {
                        ...
                        await CellDataWriter.WriteAsync(cell, dataType, writer, Settings, accessibilitySettings, addRowScope, image);
                    }
                    else
                    {
        ...
        public async Task<string> GetSinglePageAsync(string htmlDocument = "<!DOCTYPE html>\r\n<html>\r\n<head>\r\n<style type=\"text/css\">\r\n{1}</style></head>\r\n<body>\r\n{0}</body>\r\n</html>")
        {
            ...
            CssTableExporterAsync? cssExporter = HtmlExporterFactory.CreateCssExporterTableAsync(_settings, _table, _styleCache);
