...
{
    public virtual IEnumerable<ExcelDoubleCellValue> ConvertArgs(bool ignoreHidden,
                                                                 bool ignoreErrors,
                                      ...
                                      (arg, argList) =>
                                      {
                                          if (arg.IsExcelRange)
                                          {
                                              foreach (ICellInfo? cell in arg.ValueAsRangeInfo)
                                              {
                                                  ...
                                                  if (!CellStateHelper.ShouldIgnore(ignoreHidden, ignoreNonNumeric, cell, context)
                                                      && ConvertUtil.IsNumericOrDate(cell.Value))
                                                  {
                                      ...
                                      (arg, argList) =>
                                      {
                                          ...
                                          if (arg.Value is IRangeInfo)
                                          {
                                              foreach (ICellInfo? cell in (IRangeInfo)arg.Value)
                                              {
                                                  ...
                                                  {
                                                      ExcelDoubleCellValue val =
                                                          new ExcelDoubleCellValue(cell.ValueDoubleLogical, cell.Row, cell.Column);
