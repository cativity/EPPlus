...
public class ExcelPivotTableField : XmlHelper
{
    ...
    public bool IsRowField
    {
        ...
        internal set
        {
            ...
            {
                XmlElement node =
                    this.TopNode.SelectSingleNode(string.Format("../../d:rowFields/d:field[@x={0}]", this.Index), this.NameSpaceManager) as XmlElement;
    ...
    public bool IsColumnField
    {
        ...
        internal set
        {
            ...
            {
                XmlElement node =
                    this.TopNode.SelectSingleNode(string.Format("../../d:colFields/d:field[@x={0}]", this.Index), this.NameSpaceManager) as XmlElement;
    ...
    public bool IsPageField
    {
        ...
        internal set
        {
            ...
            else
            {
                ...
                XmlElement node =
                    this.TopNode.SelectSingleNode(string.Format("../../d:pageFields/d:pageField[@fld={0}]", this.Index), this.NameSpaceManager) as XmlElement;
                ...
                foreach (ExcelWorksheet? ws in this._pivotTable.WorkSheet.Workbook.Worksheets)
                {
                    foreach (ExcelDrawing? d in ws.Drawings)
                    {
                        if (d is ExcelPivotTableSlicer s
                            && s.Cache != null
                            && s.Cache.PivotTables.Contains(this._pivotTable)
                            && this.Index == s.Cache._field.Index)
                        {
                            this._slicer = s;
