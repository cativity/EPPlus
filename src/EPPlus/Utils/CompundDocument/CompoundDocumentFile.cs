...
internal partial class CompoundDocumentFile : IDisposable
{
    ...
    public void Write(MemoryStream ms)
    {
        ...
        WritePosition(bw,
                      new int[] { FAT_SECTOR, END_OF_CHAIN, END_OF_CHAIN },
                      ref this._currentFATSectorPos); //First sector is first FAT sector, second is First Dir sector, thirs is first Mini FAT sector.
    ...
    private void CheckUpdateDIFAT(BinaryWriter bw)
    {
        if (bw.BaseStream.Position % this._sectorSize == 0)
        {
            ...
            if (this._currentDIFATSectorPos == this._sectorSize
                || ((this._currentDIFATSectorPos + 4) % this._sectorSize == 0 && this._currentDIFATSectorPos > this._sectorSize))
            {
